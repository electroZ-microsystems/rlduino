...
void microrl_set_human(unsigned char h)
{
   human = h;  
   //if (human == 0)
   //   strcpy(prompt_default,"");
   //if (human == 1)
      //strcpy(prompt_default,_PROMPT_DEFAULT);
      //prompt_default[0] = '>';
...
unsigned char microrl_get_human(void)
{
   return human;  
}
...
{
	if (human)pThis->print (pThis->prompt_str);
}
...
void new_line_handler(microrl_t * pThis){
	...
	if (human) terminal_newline (pThis);
#ifdef _USE_HISTORY
	if (pThis->cmdlen > 0)
